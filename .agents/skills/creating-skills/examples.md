...
Use the template at `${SKILL_DIR}/scripts/migration-template.sql` when creating new migrations.

## Safety Checks

Before running any migration:
1. Run `${SKILL_DIR}/scripts/check-pending.sh` to verify pending migrations
...
Read the detailed reference files in `${SKILL_DIR}` for comprehensive details:
...
| **Scripts** | `${SKILL_DIR}/scripts/`, external tooling |
| **Reference files** | Split content, `${SKILL_DIR}/*.md`, lazy loading |
