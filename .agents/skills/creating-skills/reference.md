...
| `${SKILL_DIR}` | Absolute path to the skill's directory. Use for referencing supporting files. | SKILL.md |
...
- **Supporting `.md` files**: NOT automatically loaded. Only read when SKILL.md tells Claude to read them via `${SKILL_DIR}` references
- **Scripts/other files**: Available on disk but never auto-loaded. Referenced via `${SKILL_DIR}`
...
4. Project skills (`.agents/skills/`) and personal skills (`~/.agents/skills/`) are both searched
