...
- Simple commands (document in AGENTS.md or memory)
- General programming knowledge (Antigravity already knows this)
...
- **Project** (`.agents/skills/`) — Default. Use when the pattern is project-specific
- **Personal** (`~/.agents/skills/`) — Use when the pattern applies across all projects
...
**Description quality matters most.** Antigravity uses it for auto-activation. Include:
...
Read `${SKILL_DIR}/reference.md` for details.
...
Reference them with `${SKILL_DIR}`:
```markdown
Read `${SKILL_DIR}/api-patterns.md` for the complete reference.
```
...
- [ ] Supporting files use `${SKILL_DIR}` references
- [ ] No hardcoded paths
...
Has it appeared 2+ times? ──No──→ Save to memory instead
  ↓ Yes
Is it non-trivial (3+ steps)? ──No──→ Add to AGENTS.md or memory
  ↓ Yes
Is it project-specific? ──No──→ Create in ~/.agents/skills/
  ↓ Yes
