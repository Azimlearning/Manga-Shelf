---
name: self-healing
description: Continuously improve Antigravity's effectiveness by recognizing patterns, saving memory, creating skills, and refining project knowledge. Use when Antigravity notices repeated workflows, encounters a problem it solved before, wants to save something for future sessions, needs to create a reusable skill, or when the user asks Antigravity to improve itself, learn, remember, or get smarter over time.
...
Read the detailed reference files in `${SKILL_DIR}` for comprehensive guidance:
...
Existing skills: Check .agents/skills/ for what already exists
Project conventions: Read AGENTS.md for established rules
...
| Signal | Action | Where |
|--------|--------|-------|
| Solved a tricky problem | Save to memory topic file | `~/.gemini/antigravity/brain/*/memory/` |
| User corrected me | Update or remove incorrect memory | MEMORY.md or topic file |
| Found a codebase convention | Document in memory or AGENTS.md | Depends on team vs personal |
| Same workflow 2+ times | Create a new skill | `.agents/skills/` |
...
- Use `${SKILL_DIR}` for internal references, never hardcode paths
...
- Keep entries concise — AGENTS.md is loaded every session
- Prefer `.agents/rules/` files for path-specific conventions
...
- [ ] No duplicate information across memory, skills, and AGENTS.md
